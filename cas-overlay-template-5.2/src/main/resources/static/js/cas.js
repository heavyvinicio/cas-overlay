...
function disableEmptyInputFormSubmission() {
    var fields = $('#fm1 input[name="company"], [name="username"], [name="password"]');

    if (fields.length == 3) {
        fields.on('input', function (event) {
            var enableSubmission = $('#fm1 input[name="company"]').val().trim()
                && $('#fm1 input[name="username"]').val().trim()
...
function resourceLoadedSuccessfully() {
    $(document).ready(function () {
        ...
        $('#capslock-on').hide();
        $('#fm1 input[name="company"],input[name="username"],[name="password"]').trigger('input');
        $('#fm1 input[name="company"]').focus();
