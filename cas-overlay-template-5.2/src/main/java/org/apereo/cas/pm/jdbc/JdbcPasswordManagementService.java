...
public class JdbcPasswordManagementService extends BasePasswordManagementService {
    ...
    @Override
    public String findEmail(final String company,final String username) {
        try {
            final String email = this.jdbcTemplate.queryForObject(properties.getJdbc().getSqlFindEmail(), String.class,
                    company, username);
            if (StringUtils.isNotBlank(email) && EmailValidator.getInstance().isValid(email)) {
                return email;
            }
            LOGGER.debug("Company [{}] --- Username [{}] not found when searching for email", company, username);
            return null;
        } catch (final EmptyResultDataAccessException e) {
            LOGGER.debug("Company [{}] --- Username [{}] not found when searching for email", company, username);
            return null;
